...
if($modalita === 'modifica'){
    ...
    if(!$post || get_post_meta($post_id,'_allevatore_id',true)!=$current_user->ID){
        echo '<p>Fattrice non accessibile.</p>';
        return;
    }
}
...
<div class="am-template-allevatore">
    <div class="am-breadcrumbs">
        ...
        <a href="/dashboard-allevatori/?sezione=fattrici">Fattrici</a> › 
        <?php echo $modalita==='modifica'?'Modifica Fattrice':'Nuova Fattrice'; ?>
    </div>

    <div class="am-header">
        <h1 class="am-title"><?php echo $modalita==='modifica'?'Modifica Fattrice':'Aggiungi nuova Fattrice'; ?></h1>
        <p class="am-desc">Inserisci il nome della fattrice e salva.</p>
    </div>

    <form id="am-fattrice-form" class="am-section">
        <div class="am-form-group">
            <label>Nome Fattrice</label>
            <input type="text" name="nome_fattrice" id="nome_fattrice" value="<?php echo $nome; ?>" autocomplete="off" required>
            <div id="nome_fattrice-suggestions" class="am-autocomplete"></div>
        </div>
...
(function(){
    ...
    const form = document.getElementById('am-fattrice-form');
    ...
    // Autocomplete ANACT (fattrici)
    const input = document.getElementById('nome_fattrice');
    const container = document.getElementById('nome_fattrice-suggestions');
    let timer = null;
    input.addEventListener('input', function(){
        ...
        timer = setTimeout(async ()=>{
            try{
                const res = await fetch(`${ajaxUrl}?action=anact_autocomplete_cavalli&tipo=fattrici&term=${encodeURIComponent(q)}`);
    ...
    form.addEventListener('submit', e=>{
        ...
        data.append('action','am_save_fattrice');
        ...
        .then(json=>{
            ...
            showNotify('Fattrice salvata correttamente');
            setTimeout(()=>window.location.href='/dashboard-allevatori/?sezione=fattrici',800);
